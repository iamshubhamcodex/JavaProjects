		...
		JPanel pn = new JPanel() {
			...
			public void paint(Graphics g) {
				...
				if (!snake.getEnd()) {
					...
					g.drawString("Score: " + (snake.body.size() - 3), frame.getWidth() - 105, 20);
				} else {
					g.setColor(Color.white);
					g.setFont(new Font(Font.SANS_SERIF, Font.BOLD, 45));
					g.drawString("End of the Game", frame.getWidth() / 2 - 200, 340);
					g.setFont(new Font(Font.SANS_SERIF, 0, 28));
					g.drawString("Your Score " + (snake.body.size() - 3), frame.getWidth() / 2 - 100, 370);
					g.setFont(new Font(Font.SERIF, Font.ITALIC, 18));
					g.drawString("Press any key to Play Again", frame.getWidth() / 2 - 120, 730);
				}
			}
		};
